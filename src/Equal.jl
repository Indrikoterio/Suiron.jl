...
# bound to a number (SNumber) which is equal to 18.
...
struct Equal <: BuiltInPredicate
    type::Symbol
    terms::Vector{Unifiable}
    function Equal(left::Unifiable, right::Unifiable)
        new(:EQUAL, [left, right])
    end
end

mutable struct EqualSolutionNode <: SolutionNode
...
# parse_equal - creates an 'equal' predicate from a string.
...
#         error message
function parse_equal(str::String)::Tuple{Equal, Bool}
    infix, index = identify_infix(str)
    if infix != :EQUAL
        return Equal(Atom("=="), Atom("==")),
               "parse_equal() - Invalid infix: $infix"
    end
...
# Params: 'equal' predicate
#         knowledge base
#         parent solution
#         parent node
# Return: solution node
function get_solver(goal::Equal, kb::KnowledgeBase,
                    parent_solution::SubstitutionSet,
                    parent_node::SolutionNode)::SolutionNode
    return EqualSolutionNode(goal, kb, parent_solution,
...
function next_solution(sn::EqualSolutionNode)::Tuple{SubstitutionSet, Bool}
    ...
    if !ok
        throw(ArgumentError("Equal - Unground term: $(n.goal.terms[1])"))
    end

    term2, ok = get_ground_term(sn.parent_solution, sn.goal.terms[2])
    if !ok
        throw(ArgumentError("Equal - Unground term: $(n.goal.terms[2])"))
    end
...
function to_string(eq::Equal)::String
